...
def parser(program):
    ...
    # todo should we make a dictionary of variable symbols
    ...
    def map_symbol_to_address(my_sym):
        ...
        # todo new algorithm will go through and link all label symbols to positions before it processes any other instruction
        ...
        if is_predefined:
            ...
            for i, line in enumerate(program):
                if line == "(" + my_sym + ")": # if we find our symbol
                    address = i + 1 - len(unique_labels) # bind address to the address of the next instruction in the program
                    # len(unique_labels) should be 1 for LOOP and 2 for INFINITE_LOOP in this program
                    # unique_labels should only reflect the unique_labels that come BEFORE the @ LOOP
                    # to do this, make unique labels into a dictionary

                    # also,
                    print(my_sym)
                    print(variable_symbols)
                    print(i)
                    print("unique_labels = {0}".format(unique_labels))
